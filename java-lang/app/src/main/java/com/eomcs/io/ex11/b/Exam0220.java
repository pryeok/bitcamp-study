...
public class Exam0220 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // => 인스턴스를 저장할 때 버전번호도 함께 저장한다 (serialize) !!!
    // => 직렬화 데이터를 현재 갖고 있는 클래스를 이용하여 객체에 저장할 수 있는지 검사하기 위하여
    //    버전 번호를 확인한다. 만약 직렬화 데이터에 있는 버전 번호와 역직렬화 할 때 사용할 클래스의 버전 번호가
    //    다르다면 제대로 역직렬화를 할 수 없기 때문에 오류를 발생시킨다 (deserialize) !!!
